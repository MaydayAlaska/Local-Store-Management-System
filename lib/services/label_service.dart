...
class LabelService {
  ...
  Future<Uint8List> buildLabels({
    ...
    _validateJob(product: product, copies: copies, widthMm: widthMm, heightMm: heightMm);
  ...
  String buildBplZ({
    ...
    final price = product.salePriceCents == null ? '' : _formatBplPrice(product.salePriceCents!);
    ...
    final out = StringBuffer()
      ...
      ..writeln('^LL$height')
      ..writeln('^LH0,0')
      ..writeln('^FO$margin,$titleY^A0N,$titleFont,$titleFont^FD$title^FS');
    ...
    if (price.isNotEmpty) {
      ...
      out.writeln('^FO$priceX,$footerY^A0N,$priceFont,$priceFont^FD$price^FS');
    }
  ...
  Future<void> _sendTcp(String printerUrl, String payload) async {
    ...
    try {
      ...
      throw StateError(
        ...
        'Verifica che la BIXOLON sia accesa e collegata alla rete. Dettagli: ${error.message}',
      );
    } catch (_) {
      socket?.destroy();
      rethrow;
  ...
  void _validateJob({
    ...
    if (_printableCode(product).isEmpty) {
      throw ArgumentError('Il prodotto non ha un barcode o SKU stampabile.');
    }
  }
  ...
  static int _maxChars(int availableDots, int fontWidth) {
    ...
    return (availableDots / approximateCharacterWidth).floor().clamp(1, 80).toInt();
  }
  ...
  static String _formatBplPrice(int cents) {
    ...
    return '${negative ? '-' : ''}$euros,$decimals EUR';
  }
